...
<h3 style="padding-bottom: 15px;">Left over metiral Retrun Details</h3>

<div class="container box shadow-lg">
    <form action="dailyleftover.php" method="POST" name="staffadd">
...
<div class="container shadow-lg box">
    <a class="d-grid gap-2 btn btn-secondary" href='./dailyleftover.php'><i class="bi bi-arrow-clockwise"></i></a><br>
    ...
    <table id="example" class="table table-dark table-striped  text-center" style="width:100%">
        ...
        <tbody>
            ...
            if (!$conn) {
                ...
                $sql = "SELECT dailyleft.id, raw_metiral.name as 'rawname', raw_metiral.type as 'rawtype', dailyleft.qyt, dailyleft.date
                From dailyleft 
                LEFT JOIN raw_metiral on dailyleft.rawmetiral_id=raw_metiral.id";
                $result = $conn->query($sql);
...
if (isset($_POST["rawname"]) && $_POST["rawname"] != NULL) {
    ...
    if (!$conn) {
        ...
        $sql = "INSERT INTO dailyleft (rawmetiral_id, rawmetiral_type, qyt , date) VALUES ('" . $Rawname . "', '" . $Rawtype . "', '" . $stoccknum . "' , '" . $issueDate . "')";
        if ($conn->query($sql) === TRUE) {
            echo "<script> alert('Leftover quantity added successfully');</script>";
            $_POST["rawname"] = NULL;
    ...
    if (!$conn) {
        ...
        if ($conn->query($sql) === TRUE) {
            echo "<script> alert('stockall's quantity of Raw meterial Re-add successfully');</script>";
